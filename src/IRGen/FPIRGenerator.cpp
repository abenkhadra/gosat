...
const IRSymbol *FPIRGenerator::genNumeralIR
    ...
    if (expr.decl().decl_kind() == Z3_OP_BNUM) {
        ...
        Value *value = ConstantFP::get(builder.getDoubleTy(), std::stod(numeral_str));
        auto res_pair = insertSymbol(SymbolKind::kFP64Const, expr, value);
        return res_pair.first;
    }
    return nullptr;
...
llvm::Function *FPIRGenerator::genFunction
    ...
    auto md_node_1 = MDNode::get(*m_ctx, MDString::get(*m_ctx, "Simple C/C++ TBAA"));
    auto md_node_2 = MDNode::get(*m_ctx, {MDString::get(*m_ctx, "omnipotent char"), md_node_1, md_scalar});
    auto md_node_3 = MDNode::get(*m_ctx, {MDString::get(*m_ctx, "double"), md_node_2, md_scalar});
...
const IRSymbol *FPIRGenerator::genFuncRecursive
        (llvm::IRBuilder<> &builder, const z3::expr &expr, bool is_negated) noexcept {
    if (!expr.is_app()) {
    ...
    if (fpa_util::isFPVar(expr)) {
        ...
        auto result_iter = findSymbol(SymbolKind::kFP64Var, expr);
        if (result_iter != m_expr_sym_map.cend()) {
            return &(*result_iter).second;
        }
        using namespace llvm;
        ...
        auto idx_ptr = builder.CreateInBoundsGEP
                (llvm::cast<Value>(arg2), builder.getInt64(m_var_sym_vec.size()));
        auto loaded_val = builder.CreateAlignedLoad(idx_ptr, 8);
        loaded_val->setMetadata(llvm::LLVMContext::MD_tbaa, m_tbaa_node);
        auto result_pair = insertSymbol(SymbolKind::kFP64Var, expr, loaded_val);
...
llvm::Value *FPIRGenerator::genExprIR
        (llvm::IRBuilder<> &builder, const IRSymbol *expr_sym, std::vector<const IRSymbol *> &arg_syms) noexcept {
    using namespace llvm;
    switch (expr_sym->expr()->decl().decl_kind()) {
        ...
        case Z3_OP_FPA_EQ:
            if (expr_sym->isNegated()) {
                auto result = builder.CreateFCmpONE(arg_syms[0]->getValue(), arg_syms[1]->getValue());
                return builder.CreateSelect(result, m_const_zero, m_const_one);
            } else
                return builder.CreateCall(m_fp64_dis, {arg_syms[0]->getValue(), arg_syms[1]->getValue()});
        case Z3_OP_NOT:
        ...
        case Z3_OP_FPA_MINUS_ZERO:
            return ConstantFP::get(builder.getDoubleTy(), -0.0);
        case Z3_OP_FPA_ADD:
            return builder.CreateFAdd(arg_syms[1]->getValue(), arg_syms[2]->getValue());
        case Z3_OP_FPA_SUB:
            return builder.CreateFSub(arg_syms[1]->getValue(), arg_syms[2]->getValue());
        case Z3_OP_FPA_NEG:
            return builder.CreateFSub(ConstantFP::get(builder.getDoubleTy(), -0.0), arg_syms[0]->getValue());
        case Z3_OP_FPA_MUL:
            return builder.CreateFMul(arg_syms[1]->getValue(), arg_syms[2]->getValue());
        case Z3_OP_FPA_DIV:
            return builder.CreateFDiv(arg_syms[1]->getValue(), arg_syms[2]->getValue());
        case Z3_OP_FPA_REM:
            return builder.CreateFRem(arg_syms[1]->getValue(), arg_syms[2]->getValue());
        case Z3_OP_FPA_ABS: {
        ...
        case Z3_OP_FPA_LT:
            if (expr_sym->isNegated()) {
                auto comp_res = builder.CreateFCmpOGE(arg_syms[0]->getValue(), arg_syms[1]->getValue());
                return genBinArgCmpIR(builder, arg_syms, comp_res);
            } else {
                auto comp_res = builder.CreateFCmpOLT(arg_syms[0]->getValue(), arg_syms[1]->getValue());
                return genBinArgCmpIR2(builder, arg_syms, comp_res);
            }
        case Z3_OP_FPA_GT:
            if (expr_sym->isNegated()) {
                auto comp_res = builder.CreateFCmpOLE(arg_syms[0]->getValue(), arg_syms[1]->getValue());
                return genBinArgCmpIR(builder, arg_syms, comp_res);
            } else {
                auto comp_res = builder.CreateFCmpOGT(arg_syms[0]->getValue(), arg_syms[1]->getValue());
                return genBinArgCmpIR2(builder, arg_syms, comp_res);
            }
        case Z3_OP_FPA_LE:
            if (expr_sym->isNegated()) {
        ...
        default:
            std::string err = "Unsupported expr:" + expr_sym->expr()->decl().name().str() + "\n";
            assert(err.c_str());
            return nullptr;
    }
}
...
llvm::Value *FPIRGenerator::genBinArgCmpIR
        (llvm::IRBuilder<> &builder, std::vector<const IRSymbol *> &arg_syms, llvm::Value *comp_result) noexcept {
    using namespace llvm;
    ...
    auto call_res = builder.CreateCall(m_fp64_dis, {arg_syms[0]->getValue(), arg_syms[1]->getValue()});
...
llvm::Value *FPIRGenerator::genMultiArgAddIR
        (llvm::IRBuilder<> &builder, std::vector<const IRSymbol *> &arg_syms) noexcept {
    auto result = builder.CreateFAdd(arg_syms[0]->getValue(), arg_syms[1]->getValue());
...
llvm::Value *FPIRGenerator::genMultiArgMulIR
        (llvm::IRBuilder<> &builder, std::vector<const IRSymbol *> &arg_syms) noexcept {
    auto result = builder.CreateFMul(arg_syms[0]->getValue(), arg_syms[1]->getValue());
    for (unsigned i = 2; i < arg_syms.size(); ++i) {
...
std::pair<IRSymbol *, bool> FPIRGenerator::insertSymbol
        (const SymbolKind kind, const z3::expr &expr, llvm::Value *value) noexcept {
    if (kind != SymbolKind::kNegatedExpr) {
        auto result = m_expr_sym_map.insert
                ({expr.hash(), IRSymbol(kind, expr, value)});
        return {&(*result.first).second, result.second};
    } else {
        // XXX: would this weaken hashing?
        auto result = m_expr_sym_map.insert
                ({expr.hash() + static_cast<unsigned>(kind), IRSymbol(kind, expr, value)});
        return {&(*result.first).second, result.second};
    }
}
