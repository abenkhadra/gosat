...
public:
    ...
    void genFuncCode(const std::string &func_name, const z3::expr &expr) noexcept;
...
private:
    const Symbol *genFuncCodeRecursive(const z3::expr &expr, bool is_negated) noexcept;

    void genExprCode(const Symbol *expr_sym, std::vector<const Symbol *> &args_syms) noexcept;

    void includeFuncHeader(const std::string &func_name) noexcept;

    void includeFuncFooter() noexcept;

    std::pair<const Symbol *, bool> insertSymbol(const SymbolKind kind, const z3::expr &expr) noexcept;

    const char *getSymbolName(const SymbolKind kind, const z3::expr &expr) const noexcept;

    const Symbol *findSymbol(const SymbolKind kind, const z3::expr &expr) const noexcept;
